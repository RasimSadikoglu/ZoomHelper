...
class Settings(ttk.Frame):
    ...
    def initSettings(self):
        ...
        ttk.Label(self, text='Check For Updates', anchor='e', padding=5).grid(row=2, column=4, padx=5, pady=5, sticky='we')
        ttk.Combobox(self, textvariable=self.autoUpdate, state='readonly', values=['Never', 'Everytime', 'Daily'], width=13).grid(row=2, column=5, padx=5, pady=5, sticky='we')

        ttk.Label(self, text='Open Free Meetings', anchor='e', padding=5).grid(row=3, column=4, padx=5, pady=5, sticky='we')
        ttk.Checkbutton(self, variable=self.openFreeMeetings).grid(row=3, column=5, padx=5, pady=5, sticky='w')

        ttk.Button(self, text='Check For Updates', padding=5, command=self.checkForUpdate).grid(row=6, column=6, padx=5, pady=5, sticky='swe')
