...
class MeetingInfo(ttk.Frame):
    ...
    def initFrame(self):
        ttk.Label(
            self,
            **{"text": "Edit Meeting" if self.meeting != None else "Add New Meeting", "anchor": "center", "padding": 5},
        ...
        ttk.Button(self, **{"text": "<", "command": lambda: self.master.showMainMenu(), "padding": 10}).grid(
            row=0, column=0, padx=10, pady=15, sticky="wn"
        )

        if self.meeting != None:
            ttk.Button(self, **{"text": "Open", "command": lambda: self.meeting.open(), "padding": 10}).grid(
                row=1, column=2, padx=10, pady=15, sticky="s"
            )
...
class InfoFrame(ttk.Frame):
    ...
    def initFrame(self):
        # Info Label
        ttk.Label(self, text="Meeting Info", anchor="center", padding=5).grid(
        ...
        ttk.Button(self, text="Parse", command=lambda: self.linkParser()).grid(
        ...
        # Meeting Password
        ttk.Label(self, text="Meeting Password", anchor="e", padding=5).grid(
            row=4, column=0, padx=5, pady=5, sticky="news"
        )
        ...
        ttk.Checkbutton(
            self,
            text="(Always available, no specific date.)",
            variable=self.isFree,
            command=lambda: self.master.freeMeeting(self.isFree.get()),
            padding=5,
        ).grid(row=5, column=1, columnspan=2, sticky="w")

        # CopyURl
        ttk.Button(self, text="Copy Invite URL", command=lambda: self.copyURL(), padding=5).grid(
            row=6, column=2, padx=5, pady=5, sticky="news"
        )
    ...
    def copyURL(self):
        if self.id.get() != "" and self.password.get() != "":
            ...
            copyStatus = ttk.Label(
                self, text="Id and password have to be filled!", anchor="center", foreground="red", padding=5
            )
            copyStatus.grid(row=6, column=0, columnspan=2, padx=5, pady=5, sticky="e")
            self.after(3000, copyStatus.destroy)
...
class DateFrame(ttk.Frame):
    ...
    def initFrame(self):
        ...
        ttk.Checkbutton(self, text="Repeat Weekly", variable=self.weeklyRepeat, command=self.update).grid(
        ...
        self.yearMenu = ttk.Combobox(
            self, textvariable=self.year, state="readonly", values=list(range(2022, 2032)), width=5
        )
        ...
        ttk.Combobox(self, textvariable=self.startHour, state="readonly", values=list(range(24)), width=5).grid(
            row=4, column=0, padx=5, pady=5, sticky="w"
        )
        ttk.Combobox(self, textvariable=self.startMinute, state="readonly", values=list(range(60)), width=5).grid(
            row=4, column=1, padx=5, pady=5, sticky="we"
        )
        ttk.Combobox(self, textvariable=self.endHour, state="readonly", values=list(range(24)), width=5).grid(
            row=4, column=2, padx=5, pady=5, sticky="we"
        )
        ttk.Combobox(self, textvariable=self.endMinute, state="readonly", values=list(range(60)), width=5).grid(
    ...
    def getValues(self):
        return {
            "date": {"year": self.year.get(), "month": months.index(self.month.get()) + 1, "day": self.day.get()}
            if not self.weeklyRepeat.get()
            else None,
    ...
    def setValues(self, meeting: Meeting):
        ...
        if meeting == None:
            ...
            self.weekDay.set(
                ...
                if meeting.date == None and meeting.weekDay != None
                else weekDays[now.weekday()]
            )
